...
# -------------------------
# Static (trace-time) checks
# -------------------------
# These depend only on shape/dtype metadata (static under jit), so they're safe
...
def assert_array_ndim(x: Array, ndim: int, *, name: str = "x") -> Array:
    # Make ndim usable even if someone passes a JAX scalar (still "static" conceptually).
    ndim_arr = jnp.asarray(ndim, dtype=jnp.int32)
    x_ndim_arr = jnp.asarray(x.ndim, dtype=jnp.int32)  # x.ndim is static -> becomes a constant scalar array
...
def assert_arrays_equal_shape(
    ...
    # shape comparison is static -> boolean constant -> still OK under tracing
...
# --------------------------------
# Jittable runtime value predicates
# --------------------------------
# These return booleans / masks and are fully jittable & vmappable.
...
def within_interval_mask(
    ...
    *,
    ...
    lo = jnp.asarray(left_open)   # becomes a JAX bool[] tracer under tracing (fine)
...
# ---------------------------------------------------
# NOTE: checkify.check is *effectful* unless you wrap the *caller* in
# checkify.checkify(...). That wrapper "functionalizes" the effect so the
# whole thing remains pure and works with jit/vmap/grad. :contentReference[oaicite:1]{index=1}
...
def assert_array_within_interval(
    ...
    *,
    ...
    # Optional: include runtime values in the error message (these must be arrays/scalars)
    xmin = jnp.min(x)
    xmax = jnp.max(x)

    # NOTE: double braces to produce literal {} for .format placeholders
    checkify.check(ok, f"{name} has values outside the required interval. min={{}} max={{}}", xmin, xmax)
    return x


# -------------------------
# Convenience: jit + checks
# -------------------------
def jit_with_checks(
    fn,
    *,
    ...
    """
    Returns a jitted version of `fn` that supports checkify-based runtime checks.
    ...
    def wrapped(*args, **kwargs):
        ...
        err.throw()   # raises ValueError on host if any check failed
        return out
...
# Uses your already-implemented JAX version.
# def assert_array_float(x: Array, *, name: str = "x") -> Array: ...


_EMPTY_I32 = jnp.zeros((0,), dtype=jnp.int32)
_DEFAULT_BOUNDS = jnp.array([jnp.inf, jnp.inf], dtype=jnp.float32)  # matches the "bounds is None" behavior
...
def init_bounds_config_jax(
    ...
    *,
    transform: str = "probit",                # "logit" or "probit" (mark static under jit)
    scale: bool = True,
    diagonal: bool = True,
) -> dict[str, Array]:
    # n_dim must be static under jit because it sets array shapes.
    checkify.check(jnp.asarray(n_dim > 0), "n_dim must be a positive integer.")
    n_dim32 = jnp.asarray(n_dim, dtype=jnp.int32)

    # ---- bounds (fully JAX; supports pair or per-dim matrix) ----
    ...
    # Broadcast (2,) -> (n_dim, 2); (n_dim, 2) stays (n_dim, 2)
    bounds = jnp.broadcast_to(bounds, (n_dim, 2))
    low = bounds[:, 0]
    high = bounds[:, 1]
    checkify.check(jnp.all(low <= high), "bounds[:,0] must be <= bounds[:,1] elementwise.")

    # ---- periodic/reflective indices -> masks (no loops, no Python control flow) ----
    periodic = jnp.asarray(periodic, dtype=jnp.int32).reshape((-1,))
    reflective = jnp.asarray(reflective, dtype=jnp.int32).reshape((-1,))
    ...
    dims = jnp.arange(n_dim, dtype=jnp.int32)
    ...
    # ---- transform: validate & encode as int (no string stored in output; jit-friendly) ----
    is_logit = transform == "logit"     # static (mark transform static under jit)
    is_probit = transform == "probit"
    checkify.check(jnp.asarray(is_logit | is_probit), "transform must be 'logit' or 'probit'.")
    transform_id = jnp.asarray(is_probit, dtype=jnp.int32)  # 0=logit, 1=probit

    # ---- uninitialized parameters as NaNs (keeps everything JAX arrays) ----
    dtype = bounds.dtype
...
def masks_jax(low: Array, high: Array) -> dict[str, Array]:
    """
    Pure JAX mask creation (no checkify, no Python loops, no if/else/while).
...
def _create_masks_jax(n_dim: int, bounds: Array) -> dict[str, Array]:
    # Use your existing init_bounds_config_jax
...
def _inverse_both_jax(
    ...
    """
    Checkify-free JAX version of `_inverse_both` (both low and high finite).

    Returns:
      x : (N, K) transformed values for bounded-both dims
    ...
    transform_id = jnp.asarray(transform_id, dtype=jnp.int32)

    # Select only bounded-both dimensions
    u_sel = u[:, mask_both]        # (N, K)
    low_sel = low[mask_both]       # (K,)
    high_sel = high[mask_both]     # (K,)
    span = high_sel - low_sel      # (K,)
    log_span = jnp.log(span)       # (K,)  (invalid span will yield nan/-inf as usual)
...
def _forward_both_jax(
    ...
    *,
    ...
    """
    JAX version of `_forward_both` (both low and high finite), no checkify,
    no Python if/else/while/for loops.
    ...
    eps : float
        Clip probability to [eps, 1-eps] for numerical stability.
    ...
    def _logit_branch(p_in: Array) -> Array:
        # logit(p) = log(p) - log(1-p) (stable with log1p)
...
def _inverse_right_jax(u: Array, high: Array, mask_right: Array) -> tuple[Array, Array]:
    ...
    mask_right : Array, shape (D,), bool
        True where only the upper bound is finite.

    Returns
    -------
    x : Array, shape (N, K)
    J : Array, shape (N, K)   (diagonal log-Jacobian terms, matches original: u[:, mask_right])
    """
    u = jnp.asarray(u)
    high = jnp.asarray(high)
    ...
    x = high_sel - jnp.exp(u_sel)   # (N, K) via broadcasting
    J = u_sel                       # matches original code
    return x, J
...
def _inverse_left_jax(u: Array, low: Array, mask_left: Array) -> tuple[Array, Array]:
    ...
    mask_left : Array, shape (D,), bool
        True where only the lower bound is finite.
    ...
    J = u_sel                    # matches your original code
    return x, J
...
def _inverse_affine_jax(
    ...
    L: Array,                 # (D, D) (used if diagonal=False)
    log_det_L: Array,         # scalar (used if diagonal=False)
    diagonal: Array | bool,   # scalar bool
) -> tuple[Array, Array]:
    """
    Fully-JAX inverse affine transform, no checkify, no Python if/else/loops.
...
def _forward_affine_jax(
    ...
    """
    Fully-JAX forward affine transform. No checkify, no Python if/else/loops.
...
def _inverse_jax(
    ...
    transform_id = jnp.asarray(transform_id, dtype=jnp.int32)
    is_probit = (transform_id == 1)  # scalar bool (broadcasts)

    # ---- both-bounded branch (computed for all dims, only used where mask_both=True) ----
    span = jnp.where(mask_both[0], high - low, 1.0)           # (D,) safe where not both
...
def _forward_jax(
    ...
    *,
    ...
    """Shape-stable forward bounds transform: returns u with shape (N, D)."""
    x = jnp.asarray(x)
    low = jnp.asarray(low)
    high = jnp.asarray(high)
...
def inverse_jax(u: Array, cfg: Mapping[str, Array], masks: Mapping[str, Array]) -> tuple[Array, Array]:
    """
    Fully-JAX inverse transformation.

    Parameters
    ----------
    u : Array
        Shape (N, D) unconstrained input.
    cfg : dict-like pytree of JAX arrays
...
def forward_jax(
    ...
    *,
    ...
    """
    Fully-JAX forward transformation (NO runtime checking):
      u = bounds_forward(x)
...
def forward_jax_checked(
    ...
    *,
    ...
    """
    Same as forward_jax, but with checkify-based bounds check.
    IMPORTANT: call this via jit_with_checks(...) (not directly).
    """
...
def fit_jax(
    ...
    *,
    ...
    """
    Pure JAX fit: learns mu and (sigma if diagonal) or (cov/L/L_inv/log_det_L if full).
    Returns a NEW cfg dict (does not mutate).
    """
    x = jnp.asarray(x)

    # 1) forward bounds only (matches original: u = self._forward(x))
    u = _forward_jax(
        x,
        cfg["low"], cfg["high"],
        masks["mask_none"], masks["mask_left"], masks["mask_right"], masks["mask_both"],
    ...
    def _full_branch(_):
        ...
        # optional numerical stabilization (keep jitter=0.0 for exactness)
        cov = cov + jnp.asarray(jitter, dtype=dtype) * I

        L = jnp.linalg.cholesky(cov)

        # L_inv = inv(L) but more stable:
        L_inv = jsp.linalg.solve_triangular(L, I, lower=True)

        # log(det(L)) for triangular L
        log_det_L = jnp.sum(jnp.log(jnp.diag(L)))

        # original numpy code doesn't define sigma here; keep existing (or set to ones)
...
def apply_reflective_boundary_conditions_x_jax(
    ...
    """
    Fully-JAX reflective boundary conditions for selected dimensions.
    ...
    reflective_mask : Array, shape (D,), bool
        True for dimensions that should use reflective boundaries.
...
def apply_periodic_boundary_conditions_x_jax(
    ...
    """
    Fully-JAX periodic boundary conditions for selected dimensions.

    Matches the original Python while-loop semantics:
    ...
    def _wrap(x_in: Array) -> Array:
        ...
        # periodic only makes sense for finite bounds
        fin = jnp.isfinite(low) & jnp.isfinite(high)
        m = m & fin[None, :]

        # Safe placeholders to avoid inf/nan propagation in non-periodic dims
        x_safe = jnp.where(m, x_in, 0.0)
        ...
        # canonical wrap to [low, high)
        y = jnp.mod(x_safe - low_safe, span)              # in [0, span)
        x_wrap = low_safe + y                             # in [low, high)

        # --- match original while-loops: map positive multiples to 'high' instead of 'low'
...
def apply_boundary_conditions_x_jax(
    ...
    """
    Apply boundary conditions (periodic and/or reflective) to x in a JAX-pure way.
